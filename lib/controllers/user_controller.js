    ...
    try {
        ...
        const mailOptions = {
            ...
            html: `<h1>Please click the link below to set new password</h1>
                    <p><a href="http://www.remruatthanga.com/mystore/api/redirect?url=mystoreadmin://www.remruatthanga.com/mystore/reset_password?token=${token}">Reset password</a></p>
            `
        }
...
exports.resetPassword = async (req, res, next) => {
    try {
        ...
        const user = await User.findOne({reset_link: token});
        ...
        const hashedPassword = await hashPassword(password);
        user.updateOne({password: hashedPassword}, (err, user) => {
            if(err) {
